...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public delegate void ItemDataHandler(ILoopScrollCellBase ListItem, int index);
        public event ItemDataHandler ItemData;
        //==========LoopScrollRect==========
        ...
        {
            Debug.Log("itemTypeStart add = " + itemTypeStart);
            if (totalCount >= 0 && itemTypeStart - contentConstraintCount < 0)
            {
                return 0;
            }
            ...
            Debug.Log("itemTypeStart add after = " + itemTypeStart);
            return size;
        }

        protected float DeleteItemAtStart()
        {
            Debug.Log("itemTypeStart delete  = " + itemTypeStart);
            ...
            Debug.Log("itemTypeStart delete after = " + itemTypeStart);
            return size;
        }
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            var pointerDelta = localCursor - m_PointerStartLocalCursor;//拖拽过程中不断计算拖拽距离
        ...
        {//计算content和view bounds的差值，用于回调位置（实现弹簧效果）
