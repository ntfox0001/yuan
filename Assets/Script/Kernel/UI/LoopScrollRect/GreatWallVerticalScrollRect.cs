...
namespace UnityEngine.UI
{
    ...
    public class GreatWallVerticalScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        //==========LoopScrollRect==========
        [Tooltip("Prefab Source")]
        public GreatWallPrefabSource prefabSource;
        ...
        public void RefillCells(int offset = 0)
        {
            ...
            if (directionSign == -1)
                ...
                pos.x = 0;
            m_Content.anchoredPosition = pos;
            content.anchoredPosition += new Vector2(0, 2000);
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                if (!prefabSource.ReturnObject(oldItem))
                {
                    //size = 0;
                }
        ...
        public float PreloadDistance = 0;
        ...
        protected void Awake()
        {
            ...
            //             if (mTweenScale == null)
            //             {
            //                 Debug.LogException(new System.Exception("Need a TweenScale on ContentNode."), content);
            //             }
        ...
        {
            bool changed = false;
            if (viewBounds.min.y < contentBounds.min.y + PreloadDistance)
            {
            ...
            else if (viewBounds.min.y > contentBounds.min.y + threshold - PreloadDistance)
            {
            ...
            if (viewBounds.max.y > contentBounds.max.y - PreloadDistance)
            {
            ...
            else if (viewBounds.max.y < contentBounds.max.y - threshold + PreloadDistance)
            {
